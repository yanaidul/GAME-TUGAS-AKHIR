...
public class LevelSelectionSpeechManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI _jawaban;
    ...
    void Update()
    {
        ...
        if (_isBelokKiri)
        {
            _levelSelectionManager.IsAnswering = false;
            _isBelokKiri = false;
        ...
        lock (threadLocker)
        {
            if (startRecoButton != null)
            {
                ...
                if (!waitingForReco && micPermissionGranted && _levelSelectionManager.IsAnswering) ButtonClick();
            }
            if (_message != null)
            {
                _message = message;
